...
class Greed
{
	...
	public function objects_have_no_methods_at_all_by_default()
	{
		...
		$instance = new BasicClass();
		assert_that(get_class_methods($instance))->is_identical_to(array());
	}
	...
	public function traits_can_contain_abstract_methods()
	{
		...
		assert_that($entreprenur->address_of_ideal_house())
			->is_identical_to('101 Bayside Dr, Corona Del Mar CA.');
	...
	public function classes_can_have_new_properties_assigned_at_runtime()
	{
		...
		assert_that($entrepreneur->new_property)
			->is_identical_to("Social mobile photo filters");
	...
	{
		// task('uncomment the second definition of the Entreprenuer in Sample Classes');
	}
	...
	{
		$plant = new Plant();
		assert_that($plant->description_with_height())
			->is_identical_to('A living 2-foot tall plant');
	}

	public function this_will_use_the_most_specific_method_it_can()
	{
		$tree = new Tree();
		assert_that($tree->description_with_height())
			->is_identical_to('A living 15-foot tall plant');
	}
	...
	*/
	public function this_can_be_passed_between_classes_using_static_methods()
	{
		...
		assert_that($desc_from_plant)
			->is_identical_to('A living green plant with a fully-featured cafeteria!');
	}
