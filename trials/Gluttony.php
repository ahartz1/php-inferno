...
class Gluttony
{
	...
	public function strings_can_contain_many_things()
	{
		...
		assert_that($ciacco)->contains_string('e w');
	}
	...
	public function __and_tear_them_down__()
	{
		...
		assert_that(explode(' and ', $words))->is_equal_to(array('hog', 'pig', 'glutton'));
	}
	...
	public function preg_match_will_give_you_what_it_found_if_you_ask_nicely()
	{
		...
		assert_that($matches)->is_equal_to(array('an answer'));
	}
	...
	public function regex_will_consume_as_much_as_it_can()
	{
		...
		assert_that($matches)->is_equal_to(array('abra kadab'));
	}
	...
	*/
	...
	public function preg_match_will_only_return_a_single_match()
	{
		...
		assert_that($matches)->is_identical_to(array('a cat', 'cat'));
	}
	...
	public function preg_match_all_will_return_every_match()
	{
		...
		assert_that($matches[0])->is_identical_to(array('a cat', 'a rat', 'a bat'));	
		assert_that($matches[1])->is_identical_to(array('cat', 'rat', 'bat'));
	}
	...
	*/
	...
	{
		if (is_null($mouth_contents) || trim($mouth_contents) == 'nothing') {
			return array();
		}
		...
		return array_unique($no_commas_or_ands);
	}
